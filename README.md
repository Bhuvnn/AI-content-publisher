...
* **Dynamic Topic Generation & SQLite Tracking**: Generates original, creative poem prompts using a specialized LLM agent, utilizing historical data from SQLite to prevent repeating recent concepts.
...
* **SQLite Database Integration**: Persists generated content (topic, title, body, critic score, iteration, publication message ID, timestamp) using SQLAlchemy.
...
│   ├── crud.py         # DB insert/query functions
│   ├── database.py     # SQLAlchemy engine & session config
│   ├── init_db.py      # Table creation initializer
│   └── models.py       # SQLAlchemy Poem model definition
...
| `DATABASE_URL` | String | Connection string for the SQLite database (e.g. `sqlite:///database.db`). |
